...
const analyzeBlock = async (blk) => {
  ...
  for (let i = 0; i < txs.length; ++i) {
    ...
    for (
      let decodedLogIt = 0;
      decodedLogIt < decodedLogs.length;
      ++decodedLogIt
    ) {
      ...
      try {
        ...
        if (
          ...
          continue;
        if (!match(token0, WETH) && !match(token1, WETH)) continue;
      } catch (e) {
        continue;
      }
      if (decodedLog.name == "Swap") {
        ...
        const TotalSupplyToShow = ethers.utils.formatUnits(
          TotalSupply,
        ...
        const liquidity = match(token0, WETH)
          ? reserve0.mul(2)
          : reserve1.mul(2);
        const liquidityShow =
          parseFloat(
        ...
        const teamFinance = await UniswapV2PairContract.attach(pair).balanceOf(
          TeamFinance
        );
        const pinklock = await UniswapV2PairContract.attach(pair).balanceOf(
          Pinklock
        );
        const unicrypt = await UniswapV2PairContract.attach(pair).balanceOf(
          Unicrypt
        );
        const deadLock1 = await UniswapV2PairContract.attach(pair).balanceOf(
          DEAD1
        );
        const deadLock2 = await UniswapV2PairContract.attach(pair).balanceOf(
          DEAD2
        );
        const totalLocked = teamFinance
          .add(pinklock)
          .add(unicrypt)
          .add(deadLock1)
          .add(deadLock2);
